...
while (i < systemElements.length) {
    ...
    if (i==1) { //gerarchia
        ...
        try {
            ...
            fs.writeFileSync(process.env.OUTPUT_FILE, data1+"\n\n"+data2);
        } catch (err) {
            console.error(`Errore durante il salvataggio del file di backup dell'output: ${err}`);
        }

        await createModel(modelfromElements[i],systemElements[i], null);
        await sendPrompt(process.env.OLLAMA_MODEL_TO, process.env.OLLAMA_PROMPT, process.env.OUTPUT_FILE);
    } 
    i++;
